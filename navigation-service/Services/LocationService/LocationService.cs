...
namespace navigation_service.Services.LocationService
{
    public class LocationService(HttpClient httpClient, IMapper mapper, IConfiguration configuration) : ILocationService
    {
        ...
        {
            string filter = "&filter=countrycode:fr";
            
            HttpResponseMessage response = await httpClient.GetAsync($"{_geoapifyUrl}/v1/geocode/search?text={textLocation}&format=json&apiKey={_geoapifyApiKey}{filter}");
